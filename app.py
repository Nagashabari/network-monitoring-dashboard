...
HOST_PATTERN = re.compile(
    r"^(?:-{0,1}[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)*"
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(future_map):
            ...
            if not check.get("online"):
                check["alert"] = "🔴 Device is OFFLINE"

            elif (
                check.get("packet_loss") is not None
                and check["packet_loss"] > 20
            ):
                check["alert"] = "🟠 High Packet Loss"

            elif (
                check.get("latency_ms") is not None
                and check["latency_ms"] > 200
            ):
                check["alert"] = "🟡 High Latency"

            else:
                check["alert"] = "🟢 Network is Healthy"
...
def api_history(target_id: int):
    ...
    return jsonify(
        {
            "target_id": target_id,
            "history": db.history(target_id, limit=limit),
        }
    )
